...
private:
    ...
    double *m_ptr;//数组指针    
public:
    Matrix(unsigned r, unsigned c) :m_row(r), m_col(c)//非方阵构造     
    {
        m_size = r*c;
        if (m_size>0)
        {
    ...
    Matrix(unsigned r, unsigned c, double val ) :m_row(r), m_col(c)// 赋初值val  
    {
        m_size = r*c;
        if (m_size>0)
        {
    ...
    Matrix(unsigned n) :m_row(n), m_col(n)//方阵构造      
    {
        m_size = n*n;
        if (m_size>0)
        {
    ...
    Matrix getrow(unsigned index); // 返回第index 行,索引从0 算起
    Matrix getcol(unsigned index); // 返回第index 列
    ...
    double norm2();//2范数   
    double mean();// 矩阵均值  
    double*operator[](int i){ return m_ptr + i*m_col; }//注意this加括号， (*this)[i][j]   
...
Matrix Matrix::mtanh() // 对应元素 tanh()
{
    ...
    for (unsigned i = 0; i<ret.m_size; i++)
    {
...
Matrix Matrix::getrow(unsigned index)//返回行
{
...
Matrix Matrix::getcol(unsigned index)//返回列
{
...
{
    for (unsigned i = 0; i < obj.m_row; i++) //打印逆矩阵        
    {
        for (unsigned j = 0; j < obj.m_col; j++)
        {
            out << (obj[i][j]) << "\t";
        }
        out << endl;
...
{
    for (unsigned i = 0; i < obj.m_row; i++)      
    {
        for (unsigned j = 0; j < obj.m_col; j++)
        {
            out << (obj[i][j]) << "\t";
...
Matrix operator*(const Matrix& lm, const Matrix& rm)  //矩阵乘法
{
    ...
    for (unsigned i = 0; i<lm.m_row; i++)
    {
        for (unsigned j = 0; j< rm.m_col; j++)
        {
            for (unsigned k = 0; k< lm.m_col; k++)//lm.m_col == rm.m_row      
            {
...
Matrix Matrix::eig_vect(_In_opt_ unsigned _iters)
{
    ...
    for (unsigned count = 0; count < NUM; ++count)
    {
        ...
        *this = copym;
        ...
        for (unsigned i = 0; i < m_col; ++i)//A-lambda*I       
        {
            m_ptr[i * m_col + i] -= eValue;
        }
        //cout<<*this<<endl;      
        //将 this为阶梯型的上三角矩阵        
        for (unsigned i = 0; i < m_row - 1; ++i)
        {
            diag = m_ptr[i*m_col + i];  //提取对角元素    
            for (unsigned j = i; j < m_col; ++j)
            {
                m_ptr[i*m_col + j] /= diag; //【i,i】元素变为1    
            }
            for (unsigned j = i + 1; j<m_row; ++j)
            {
                diag = m_ptr[j *  m_col + i];
                for (unsigned q = i; q < m_col; ++q)//消去第i+1行的第i个元素    
                {
        ...
        midSum = sqrt(midSum);
        for (unsigned i = 0; i < ret.m_row; ++i)
        {
...
Matrix Matrix::cov(bool flag)
{
    ...
    for (unsigned j = 0; j<m_col; j++) //init    
    {
        mean[j] = 0.0;
    }
    Matrix ret(m_col);
    for (unsigned j = 0; j<m_col; j++) //mean    
    {
        for (unsigned i = 0; i<m_row; i++)
        {
            mean[j] += m_ptr[i*m_col + j];
    ...
    for (i = 0; i<m_col; i++) //第一个变量    
    {
        for (j = i; j<m_col; j++) //第二个变量    
        {
            ...
            /*temp.Format("cov=%f,column=%d mean(%d)=%f,mean(%d)=%f",cov[i*column+j],m_row,i,mean[i],j,mean[j]);
            MessageBox(temp);*/
        }
    }
 ...
 * 返回代数余子式
 */
double CalcAlgebraicCofactor( Matrix& srcMat, int ai, int aj)
{
...
Matrix Matrix::adjoint()
{
    ...
    for (int ai = 0; ai < m_row; ai++)
    {
        for (int aj = 0; aj < m_row; aj++)
        {
